import { CommandSpec } from "@/lib/commands";
...
export const commands: CommandSpec[] = [
  ...
  // External links
  {
    ...
    handler: async ([target]) => {
      const linkMap: Record<string, string> = {
        ...
        github: "https://github.com/danny-oko",
      };
      ...
      if (!url) {
        return {
          ...
          payload: {
            ...
            description: `No link for '${target}'. Available: ${Object.keys(
              linkMap
            ).join(", ")}`,
          },
        };
  ...
  // Theme control
  {
    ...
    handler: async ([preset]) => {
      ...
      if (validThemes.includes(lowerPreset)) {
        ...
        return {
          type: "text",
          payload: `Invalid option. Available themes: ${validThemes.join(
            ", "
          )}. Available wallpapers: ${validWallpapers.join(", ")}`,
        };
      }
    },
  },
  ...
  // Contact commands
  {
    ...
    usage:
      "contact [email|phone|github|instagram|facebook|book] | contact copy <email|phone>",
    handler: async (args) => {
      const [sub, sub2] = args;
      ...
      const contactMap: Record<string, () => any> = {
        email: () => open("mailto:danny.otgontsetseg@gmai.com"),
        phone: () => open("tel:+97680296007"),
        github: () => open("https://github.com/danny-oko"),
      ...
      if (sub === "copy") {
        if (sub2 === "phone") {
          ...
          return copy("danny.otgontsetseg@gmai.com");
        } else {
          return {
            type: "text",
